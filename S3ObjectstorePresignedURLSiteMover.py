...
from EventRangesPandaProxy import  envPandaServerURL, envPandaProxyURL
...
class S3ObjectstorePresignedURLSiteMover(SiteMover.SiteMover):
    """ SiteMover that uses panda proxy """
    # no registration is done
    copyCommand = "S3ObjectstorePresignedURL"
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(S3ObjectstorePresignedURLSiteMover, cls).__new__(cls, *args, **kwargs)
    ...
    def stageInFile(self, source, destination, sourceSize, sourceChecksum):
        """StageIn the file. should be implemented by different site mover."""
        try:
    ...
    def stageOutFile(self, source, destination, sourceSize, sourceChecksum, outputDir=None, timeout=3600):
        """StageOut the file. should be implemented by different site mover."""
        try:
    ...
    def stageOut(self, source, jobId, lfn, jobSetID, pandaProxySecretKey, experiment=None, outputDir=None, timeout=3600):
        """Stage in the source file"""
        self.log("Starting to stageout file %s with presigned url" % (source))
        ...
        status, output = self.stageOutFile(source, destination, localSize, localChecksum, outputDir=outputDir, timeout=timeout)
