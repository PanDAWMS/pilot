...
# self.__queuename is not needed
    ...
    def cleanup(self, rf=None):
        ...
        if os.path.isdir(self.__job.workdir):
            ...
            try:
                copy2("%s/metadata-%d.xml" % (self.__job.workdir, self.__job.jobId), "%s/metadata-%d.xml" % (self.__pworkdir, self.__job.jobId))
            except Exception, e:
...
#        runJob.setMessageThread(message_thread)
        ...
        if job.result[1] != 0 or job.result[2] != 0:
            self.failJob(job.result[1], job.result[2], job, runJob.getPilotServer(), runJob.getPilotPort(), pilotErrorDiag=job.pilotErrorDiag)
        runJob.setJob(job)
