    ...
    def executePayload(self, thisExperiment, runCommandList, job):
        ...
        for cmd in runCommandList:
            ...
            try:
                ...
                to_script =  to_script + "\n" + "\n".join(setup_commands)
                to_script = "%s\naprun -n %s %s %s" % (to_script, cpu_number, cmd["payload"], cmd["parameters"])    
                ...
                try:
                    ...
                    if fork_job.state == saga.job.PENDING:
                        fork_job.cancel()
                        tolog("Wait time (%s s.) exceed, job canceled" % self.waittime)
...
if __name__ == "__main__":
    ...
    # Setup HPC specific parameters for RunJob
    
    runJob.cpu_number_per_node = 16
    runJob.walltime = 60
    runJob.max_nodes = None 
    runJob.number_of_threads = 1
    runJob.min_walltime = 50 * 60
    runJob.waittime = 7 * 60
    runJob.nodes = 1
