...
#            ec = self.replaceQueuedataField("copytoolin", "gfal-copy")
        ...
        if thisSite.sitename == "NERSC_Edison" and not 'pdsf' in hostname:
            ...
            ec = self.replaceQueuedataField("catchall", "HPC_HPC,log_to_objectstore,yoda_to_zip,plugin=slurm,mode=normal,queue=regular,backfill_queue=regular,max_events=500000,initialtime_m=3,time_per_event_m=4,repo=m2015,nodes=50,min_nodes=1,max_nodes=2001,partition=edison,min_walltime_m=59,walltime_m=170,max_walltime_m=180,cpu_per_node=24,mppnppn=1,ATHENA_PROC_NUMBER=24,stageout_threads=20,copy_input_files=false,parallel_jobs=2")
