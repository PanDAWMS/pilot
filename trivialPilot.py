...
import sys, os, time, urllib2, cgi, commands, re, getopt, signal
...
def search_stage_opts(opts):
    ...
    for opt in list_opts:
        ...
        if opt.startswith('--pilotStageInSource'):
        ...
        if opt.startswith('--pilotStageOutProtocol'):
        ...
        #New variables for the new parameters for Globus Online
...
def stageincmdGO(src, dest):
    ...
    #Create transfer task
    cmd = 'gsissh cli.globusonline.org scp -v %s %s:%s'%(src, dest, jobdir)
...
print "trivialPilot.py: Running python from",sys.prefix,", platform", sys.platform, ", version", sys.version
try:
...
if out.find('https') < 0:
    print "trivialPilot: ERROR: curl not found or not https-capable. Aborting."
...
def runJob():
    ...
    print "Endpoint for Worknode:"
    print endpoint
    if endpoint:
           pass
        #print "Retrieve user proxy credential"
    ...
    if endpoint:
           pass
        #Removes Globus Connect and Endpoint
        #stopGC()
    	#time.sleep(20)
...
def receive_signal(signum, stack):
    print 'Pilot Received Signal:', signum
    sys.stdout.flush()
    shutdown(signum)
    sys.exit(-1)
